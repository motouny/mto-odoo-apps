...
const STORAGE_KEY = "sa_dga_theme_sidebar_collapsed";
...
export class AppSidebar extends Component {
    static template = "sa_dga_theme.AppSidebar";
    static props = {};

    setup() {
        this.menuService = useService("menu");
        this.actionService = useService("action");
        // Settings > DGA Theme > Backend Layout - exposed via
        // ir.http.session_info() (models/ir_http.py) so it's available
        // synchronously at boot, before any ORM call could resolve.
        this.enabled = Boolean(session.sa_dga_sidebar_enabled);
        this.state = useState({
        ...
        if (!this.enabled) {
            ...
            document.body.classList.add("o_dga_sidebar_disabled");
        }
        ...
        useEffect(
            (enabled, collapsed, isSmall) => {
                ...
                classList.toggle("o_dga_sidebar_collapsed", collapsed);
                classList.toggle("o_dga_sidebar_hidden", isSmall);
                return () => {
                    classList.remove("o_dga_sidebar_collapsed", "o_dga_sidebar_hidden");
                };
            },
    ...
    onOpenHomeMenu() {
        this.actionService.doAction("sa_dga_theme.action_dga_home_menu");
    }
...
registry.category("main_components").add("sa_dga.AppSidebar", { Component: AppSidebar });
