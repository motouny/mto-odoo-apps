...
12. **Reports**: Asset Register (سجل الأصول), Classification & Coding Sheet, Physical Count Sheet, Asset Tag Label (QR) — all QWeb, `binding_type: report` on the relevant models
13. **Automated Tests**: 8 tests in `tests/test_sa_gov_asset.py` — capitalization-threshold determination (land/NA, above/below threshold, large-entity override), manual override, MOF asset code generation, depreciation schedule generation and math (including a regression test that posting a depreciation line correctly updates `book_value`/`accumulated_depreciation`), useful-life range validation, disposal-method guard
...
21. **ZIP File Path**: `marketplace_addons/dist/sa_gov_fixed_assets_18.0.zip` (54 files, ~1.9 MB, module folder at the archive root, no `__pycache__`/`.pyc`/`.DS_Store`)
...
24. **Release Checklist**:
    ...
    - [x] Full automated test suite passing (8/8), both scoped and inside the unfiltered core suite
